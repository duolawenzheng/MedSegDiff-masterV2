...
class TrainLoop:
    ...
    def run_loop(self):
        # 初始化一个计数器
        i = 0
        # 创建数据迭代器，用于从数据加载器dataloader中获取数据批次
        data_iter = iter(self.dataloader)
        while (
            not self.lr_anneal_steps
            or self.step + self.resume_step < self.lr_anneal_steps
        ):
            # 尝试从数据迭代器中获取下一个数据批次，如果遇到 StopIteration 异常，则说明数据集已经遍历完，需要重新初始化数据迭代器以进行下一轮训练。
            try:
            ...
            # 接受数据批次 batch 和条件 cond 并执行模型的前向和反向传播，进行一步训练。
    ...
    def run_step(self, batch, cond):
        # 将 cond（假设是张量）按通道维度（dim=1）拼接到 batch 中。这通常用于将条件信息与输入数据合并
        batch = th.cat((batch, cond), dim=1)

        cond = {}
        # 将 cond（假设是张量）按列（dim=1）拼接到 batch 中。这通常用于将条件信息与输入数据合并
    ...
    def forward_backward(self, batch, cond):
        ...
        # 从 0 开始，以步长 self.microbatch 遍历到 batch.shape[0]
        for i in range(0, batch.shape[0], self.microbatch):
            # 整个批次数据中提取一个微批次  micro是一个4 4 64 64的张量
            ...
            # functools.partial 函数，它的目的是创建一个新的可调用函数，这个新函数部分地应用了 self.diffusion.training_losses_segmentation 函数，并固定了一些参数
            compute_losses = functools.partial(
                # 传递一个计算损失的方法  方法的返回值是损失和输出的张量
            ...
            if last_batch or not self.use_ddp:
                # losses1是个列表  第一项包括loss_diff   loss_cal   loss  第二项是个张量
